...
class _PlattScaling():
    ...
    def __init__(self, seed:int = 1, verbose_train:bool = False, verbose_test:bool = False):
        self.seed = seed
        self.verbose_train = verbose_train
        self.verbose_test = verbose_test
        self.model = LogisticRegression(seed = self.seed, verbose_train=self.verbose_train, verbose_test=self.verbose_test)
    
    def platt_train(self, model_output:np.ndarray, Y_train:np.ndarray, alpha:float = .0001, epochs:int = 1000, metric_freq:int = None):
        '''
        ...
        '''
        self.model_output = model_output.T
        self.Y_train = Y_train
        self.alpha = alpha
        self.epochs = epochs
        self.train_loss, self.train_acc, self.__params = self.model.train(X_train = self.model_output, Y_train = self.Y_train, alpha = self.alpha, epochs = self.epochs, metric_freq = metric_freq)
        self.platt_inf(model_output = self.model_output, Y_inf = self.Y_train) 
        return self.probs, self.pred, self.train_loss, self.train_acc
    
    def platt_inf(self, model_output:np.ndarray, Y_inf:np.ndarray):
        '''
        ...
        self.Y_inf = Y_inf
        self.test_loss, self.test_acc, self.pred, self.probs = self.model.test(X_test = self.model_output, Y_test = self.Y_inf, return_probs = True) 
